...
/* Matchmaking screen: handles room ready flow and prepares duplicated teams for battle.
   The actual battle UI will be on a separate screen. */
...
export default function MatchmakingScreen() {
  ...
  const navigatedToBattle = useRef(false);
  ...
  useEffect(() => {
    const fetchTeams = async () => {
      try {
        ...
        if (isHost) {
          ...
          myBattleTeamId = battleTeamId || null;
    ...
    // navigation to BattleScreen is handled in a separate effect below
  ...
  const onMyCharPress = (char) => {
    if (battleStarted && currentTurnUserId === userId) {
      if (attackedMap[char.id]) {
        Alert.alert("Already attacked", "This character has already attacked this turn.");
        return;
      }
